...
void CGrablinkSnapshotView::OnDraw(CDC* pDC)
{
    ...
    if (pDoc->_bResizeImage) {
        ...
        StretchDIBits(pDC->GetSafeHdc(),
            ...
            0, 0, SizeX, SizeY,                  // source
            pDoc->m_pCurrent, m_pBitmapInfo, DIB_RGB_COLORS, SRCCOPY);
    } else {
    ...
    // Throttle status bar updates to ~10 Hz
    static int targetHz = 2;
    static int frameCount = 0;
    frameCount++;

    int updateInterval = (int)(frameRate_Hz / targetHz);
    if (updateInterval < 1) updateInterval = 1;

    if (frameCount >= updateInterval) {
        // Add progress counter if capturing
        if (pDoc->_numImagesCounter > 0) {
            CString progressStr;
            progressStr.Format(" - Captured: %d/%d", pDoc->_numImagesCounter, pDoc->_numImages);
