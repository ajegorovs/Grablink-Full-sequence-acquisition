...
namespace Euresys
{
  namespace MultiCam
  {
    ...
    inline void Surface::Convert(Surface& convertedSurface)
    {
      MCSTATUS status;

      status = McConvertSurface((MCHANDLE)(*this), (MCHANDLE)convertedSurface);
      if (status != MC_OK)
        ThrowMultiCamException(status, TYPE_CONVERT_SURFACE);
    }
  }
}
